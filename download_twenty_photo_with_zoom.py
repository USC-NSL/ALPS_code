...
def generate_url(addr, query_fov, addr_bearing, addr_pitch, query_key_google):
	my_url = ('https://maps.googleapis.com/maps/api/streetview?size=900x900&location=%s&fov=%s&heading=%s&pitch=%s&key=%s') % (addr, query_fov, addr_bearing, addr_pitch, query_key_google)
	return my_url
...
def download_zoom_photo(addr, bearing, photo_index, landmark_index, photo_dir, sleep_time, query_fov, f_meta, query_key_google):
...
if __name__ == '__main__':
	...
	while (f.readline()):
		...
		for i in range(5):
			...
			download_zoom_photo(caddr, caddr_bearing, photo_index, landmark_index, photo_dir, sleep_time, query_fov, f_meta, query_key_google)
			photo_index += 1
		...
		for i in range(5):
			...
			download_zoom_photo(paddr, paddr_bearing, photo_index, landmark_index, photo_dir, sleep_time, query_fov, f_meta, query_key_google)
			photo_index += 1
