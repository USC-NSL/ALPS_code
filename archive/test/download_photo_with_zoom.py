...
photo_dir = "./data/image_with_zoom/"

f_input = open("./data/generate_standing_point_output.txt", "r")
f_output = open("./data/download_photo_with_zoom_output.txt", "w")
	...
	for k in range(6):
		...
		for i in range(-20, 40, 20):
			for j in range(-5, 15, 10):
				...
				photo_name = "%s_%s_%s_%s_%s_%s_h%s_p%s.jpg" % (node3.id.zfill(12), str(way_index).zfill(2), str(node_index).zfill(2), query_lat, query_lng, query_heading_origin, query_heading_index, query_pitch_index)
